...
Personal AI email agent. Stack: Python · FastAPI · Pydantic · Gemini 1.5 Flash (primary) · Codex Haiku 4.5 (fallback) · Gmail API · Supabase · Infisical · React · PWA · AWS Lambda + SnapStart · Terraform.
...
- [.Codex/](/.Codex/) — Codex configuration (commands, plans)
...
All plan documents live in [.Codex/plans/](.Codex/plans/). Name files `YYYY-MM-DD-<slug>.md`. Never place plans at repo root or inside `backend/` or `frontend/`.
...
- [/make-docs](.Codex/commands/make-docs.md) — generate Swagger/OpenAPI spec at version `1.0.0`.
- [/test](.Codex/commands/test.md) — run Python + React test suites.
...
- **OpenAPI regen:** Always via `make docs` or [/make-docs](.Codex/commands/make-docs.md) — it pins `info.version` to `1.0.0` and regenerates `frontend/src/api/schema.d.ts`. Never hand-edit [packages/contracts/openapi.json](packages/contracts/openapi.json).
...
- **Fallback chain** — catalog-driven `[CATALOG primary, *CATALOG fallbacks]` from [backend/app/llm/catalog.py](backend/app/llm/catalog.py) (Gemini Flash primary, Codex Haiku 4.5 fallback, both routed via OpenRouter per ADR 0009).
...
- **Hard caps** — e.g. Codex Haiku 4.5 at 100 calls/day (plan §19.15).
